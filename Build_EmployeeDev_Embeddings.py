...
JSON_FILE = PROJECT_ROOT / "JsonChunksPapers" / "employeedev_kb.json"
CHROMA_DIR = PROJECT_ROOT / "ChromaEmbeddings"


# Name of the Chroma collection that will store the employee development knowledge base chunks
EMPLOYEEDEV_COLLECTION_NAME = "employeedev_kb"
...
def load_chunks() -> List[Dict[str, Any]]:
    """
    Load employeedev_kb.json and return a list of chunk records.
    ...
    if not isinstance(data, list):
        raise RuntimeError("Expected employeedev_kb.json to contain a list of chunks.\n")
    ...
    if not valid_chunks:
        raise RuntimeError("No valid chunks found in employeedev_kb.json after filtering.\n")

    print(f"Loaded {len(valid_chunks)} valid chunks from {JSON_FILE}")
    return valid_chunks


def build_employeedev_embeddings() -> None:
    """
    Read employeedev_kb.json, create embeddings for each chunk_text, and
    store them in a persistent Chroma collection named 'employeedev_kb'.
    """
    ...
    # Create or get the employeedev collection
    collection = chroma_client.get_or_create_collection(EMPLOYEEDEV_COLLECTION_NAME)

    # Clear existing data so re-running this script does not duplicate entries
    print(f"Clearing existing data in Chroma collection '{EMPLOYEEDEV_COLLECTION_NAME}'...\n")
    ...
    print(f"Storing embeddings in: {CHROMA_DIR} (collection: {EMPLOYEEDEV_COLLECTION_NAME})\n")
    ...
    print(f"\nDone. Stored {total} embedded chunks in collection '{EMPLOYEEDEV_COLLECTION_NAME}'.")
    print(f"Chroma persistent directory: {CHROMA_DIR}\n")


if __name__ == "__main__":
    build_employeedev_embeddings()
